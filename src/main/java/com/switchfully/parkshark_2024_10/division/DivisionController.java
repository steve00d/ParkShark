...
public class DivisionController {
    ...
    public void addDivision(@RequestBody CreateDivisionDto createDivisionDto, @RequestHeader("Authorization") String token) {
        Person user = authService.userAuthenticated(token);
        if (!user.hasPermission(Permission.CAN_CREATE_DIVISIONS)) {
            throw new UnauthorizedException();
        }
        divisionService.addDivision(createDivisionDto);
    }
}
